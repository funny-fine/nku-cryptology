...
int main()
{
    ...
    int num[10]; for (int i = 0; i < 8; i++) num[i]=rand() % 56;
    while (ii<10)
    {
        ...
        for (int k = 0; k < 8; k++)
        {
            ...
            memset(key, 0, sizeof(key)); memset(key_56, 0, sizeof(key_56)); memset(temp, 0, sizeof(temp));
            memset(kidkey, 0, sizeof(kidkey)); memset(mkidkey, 0, sizeof(mkidkey));
